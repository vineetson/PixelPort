...
const ConnectSection: React.FC<{ position?: [number, number, number] }> = ({ position = [0, 0, 0] }) => (
  <group position={position}>
    ...
    <Html position={[0, 0.1, 0.2]} center>
      <div style={retroBoxStyle}>
        ...
        {/* Contact Information */}
        <div style={{ marginTop: "12px", ...labelStyle }}>Contact Info</div>
        ...
        {/* Portfolio Links */}
        <div style={{ marginTop: "12px", ...labelStyle }}>Portfolio Links</div>
        <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
          {portfolioLinks.map(link => (
            <li key={link.label} style={{ margin: "8px 0" }}>
              <a
                href={link.url}
                target="_blank"
        ...
        <div style={{ marginTop: "12px", ...labelStyle }}>Let's Build Something Amazing!</div>
      </div>
    </Html>
  </group>
);
